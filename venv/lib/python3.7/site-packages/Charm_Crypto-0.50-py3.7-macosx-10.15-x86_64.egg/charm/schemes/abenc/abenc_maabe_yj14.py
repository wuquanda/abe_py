...
class MAABE(object):
    ...
    def ukeygen(self, GPP, authority, attribute, users):
        ...
        UKs={}
        for userId in users:
            u_uid = users[userId]['sk']
            UKs[userId] = GPP['H'](attribute) ** (ASK['beta'] * (newVersionKey - oldVersionKey) * (u_uid + ASK['gamma']))
...
def revokedTest():
    ...
    maabe.setupAuthority(GPP, authority1, authorityAttributes, authorities)
    y=json.dumps(authorityAttributes);
    alice = { 'id': 'alice', 'authoritySecretKeys': {}, 'keys': None }
    alice['keys'], users[alice['id']] = maabe.registerUser(GPP)
